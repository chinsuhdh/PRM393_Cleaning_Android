...
final bookingServiceDetailProvider = FutureProvider.autoDispose
    .family<Map<String, dynamic>, String>((ref, id) async {
      try {
        final response = await DioClient.instance.get(
          '/ServiceCatalog/services/$id',
        );
...
class _CreateBookingScreenState extends ConsumerState<CreateBookingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final serviceAsync = ref.watch(
      bookingServiceDetailProvider(widget.serviceId),
    );
                          ...
                          try {
                            ...
                            final payload = {
                              ...
                              "scheduledTime": scheduledTime
                                  .toUtc()
                                  .toIso8601String(),
                              "durationHours": 2, // Có thể làm input sau
                              "quantity": 1, // Có thể làm input sau
                            ...
                            await DioClient.instance.post(
                              '/Ai/match-worker/$realBookingId',
                            );
                            ref.invalidate(bookingsProvider);
                            if (mounted)
                              context.push('/finding-worker/$realBookingId');
                          } catch (e) {
                            if (mounted)
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
  ...
  Widget _buildSummaryStep(AsyncValue<Map<String, dynamic>> serviceAsync) {
    return serviceAsync.when(
      ...
      data: (service) {
        final addressText =
            _selectedAddress?['addressText'] ?? 'Chưa chọn địa chỉ';
