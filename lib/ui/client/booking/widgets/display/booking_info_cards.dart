...
List<Widget> buildBookingDetailContent(ThemeData theme, Booking booking, UserRole role) {
  ...
  return [
    ...
    BookingInfoCard(title: 'Thông tin đặt lịch', rows: bookingTripInfoRows(booking, role)),
    const SizedBox(height: 16),
...
List<(String, String)> bookingTripInfoRows(Booking booking, UserRole role) => [
      (
        'Ngày & Giờ',
        booking.isImmediate
            ? 'Ngay bây giờ'
      ...
      if (role == UserRole.worker && (booking.distanceKm != null || booking.estimatedMinutes != null))
        ('Khoảng cách', _distanceAndEta(booking)),
      if (booking.notes.isNotEmpty) ('Ghi chú', booking.notes),
    ];

String _distanceAndEta(Booking booking) {
  final parts = <String>[
    if (booking.distanceKm != null) '${booking.distanceKm!.toStringAsFixed(1)} km',
    if (booking.estimatedMinutes != null) formatDuration(Duration(minutes: booking.estimatedMinutes!.round())),
  ];
  return parts.join(' · ');
}
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(vndFormat.format(booking.price),
  ...
  Widget _line(ThemeData theme, String label, double amount) => Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            Text('${amount < 0 ? '-' : ''}${vndFormat.format(amount.abs())}'),
          ],
        ),
      );
}
