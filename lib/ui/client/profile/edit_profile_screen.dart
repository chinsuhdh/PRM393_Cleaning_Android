...
class _EditProfileScreenState extends ConsumerState<EditProfileScreen> {
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      final avatarUrl = await ref.read(profileRepositoryProvider).uploadAvatar(pickedFile.path, pickedFile.name);
      ...
      showAppSuccessSnackBar(context, 'Cập nhật ảnh đại diện thành công!');
  ...
  Future<void> _handleVerifyPhoneFlow() async {
    try {
      await ref.read(profileRepositoryProvider).sendPhoneVerificationOtp();
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                try {
                  await ref.read(profileRepositoryProvider).verifyPhone(
                  ...
                  showAppSuccessSnackBar(context, 'Xác thực số điện thoại thành công!');
                } on AppException catch (e) {
                  if (!mounted) return;
                  showAppErrorSnackBar(context, e);
                }
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      ref.invalidate(myProfileProvider);
      showAppSuccessSnackBar(context, 'Cập nhật hồ sơ thành công!');
      context.pop();
    } on AppException catch (e) {
      if (!mounted) return;
      showAppErrorSnackBar(context, e);
